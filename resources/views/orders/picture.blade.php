...
@section('content')
    ...
    <h1 class="text-center form-h mt-5 mb-4">Оформление заказа на картину</h1>
    <div class="d-flex justify-content-center mt-5">
        <form action="{{route('picture.order.store')}}" method="POST">
            @csrf
            <div class="form-inputs d-flex flex-column">
                <select name="service" id="service" class="form-input mb-4 m-auto" required aria-label="Default select example">
                    <option selected>Выберете тип услуги</option>
                    <option value="Скетчинг">Скетчинг</option>
                    <option value="Личные уроки">Личные уроки</option>
                    <option value="Мастер класс">Мастер класс</option>
                    <option value="Картина в наличии">Картина в наличии</option>
                    <option value="Картина под заказ">Картина под заказ</option>
                </select>
